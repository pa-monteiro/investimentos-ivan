...
class PaymentRepository implements IPaymentRepository {
  ...
  async listValueTotalAndValueAvailableByProduct(user_id: string) {
    const valueTotal = await this.paymentsUserRepository
      .createQueryBuilder("pay")
      .select("pay.product_id, pay.value as valueTotal")
      .where("user_id = :user_id", { user_id })
      .execute();

    const valueAvailable = await this.repository
      .createQueryBuilder()
      .select("*")
      .where("type = :type", { type: "entries" })
      .andWhere("status = :status", { status: "accepted" })
      .andWhere("release_date <= :release_date ::date", {
        release_date: dayjs(),
      })
      .execute();
  ...
  async getValueTotalOrByProductId(
    user_id: string,
    product_id?: string
  ): Promise<number> {
    const payment = await this.paymentsUserRepository
      .createQueryBuilder()
    ...
    if (product_id) {
      const payment = await this.repository
        ...
        .where(
          "user_id = :user_id AND product_id = :product_id AND release_date => :dateNow ::date",
          {
            user_id,
            product_id,
            dateNow: dayjs(),
  ...
  async findById(id: string): Promise<Payment> {
    ...
    if (payment.start_date) {
      payment.release_date = new Date(
        dayjs(payment.release_date).format("DD/MM/YYYY")
      );
      payment.start_date = new Date(
        dayjs(payment.start_date).format("DD/MM/YYYY")
      );
    }
  ...
  async findByUserId(id: string) {
    const allValues = await this.repository
      .createQueryBuilder("payments")
      .select([
        "payments.id",
        "payments.type",
  ...
  async accept(id: string, user_id: string): Promise<Payment> {
    ...
    payment.start_date = new Date(
      dayjs()
        .add(payment.product.deadline_contribution, "day")
        .tz("America/Sao_Paulo")
        .format("YYYY-MM-DD")
    );
    payment.release_date = new Date(
      dayjs()
        .add(payment.product.withdrawal_deadline, "day")
        .tz("America/Sao_Paulo")
        .format("YYYY-MM-DD")
    );
    ...
    const valorTotalDoFundo = paymentsUsers.reduce(
      (acc, val) => acc + Number(val.value),
      0
    );

    if (paymentUser) {
      paymentUser.value += payment.value;
      const valorTotalAtualizado =
        Number(valorTotalDoFundo) + Number(paymentUser.value);
      paymentUser.percentage_by_product =
        (paymentUser.value / valorTotalAtualizado) * 100;
      await this.paymentsUserRepository.save(paymentUser);
    } else {
      const valorTotalAtualizado =
        Number(valorTotalDoFundo) + Number(payment.value);
      const transaction = this.paymentsUserRepository.create({
        product_id: payment.product_id,
        user_id: payment.user_id,
        percentage_by_product:
          valorTotalDoFundo === 0
            ? 100
            : (payment.value / valorTotalAtualizado) * 100,
        value: payment.value,
      });
    ...
    paymentsUsers.forEach(async (e) => {
      await this.paymentsUserRepository.update(e.id, {
        percentage_by_product:
          valorTotalDoFundo === 0
            ? 100
            : (e.value / valorTotalAtualizado) * 100,
      });
    });
  ...
  async filterByUserIdAndProduct(user_id: string, product_id: string) {
    ...
    return await this.repository
      ...
      .andWhere("payments.status = :status OR payments.status = :status2", {
        status: "accepted",
        status2: "pending",
      })
